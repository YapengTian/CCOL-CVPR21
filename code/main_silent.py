...
class NetWrapper(torch.nn.Module):
    ...
    def forward(self, batch_data, args):
        ...
        if args.testing:
            ...
            pred_masks[0] = torch.mul(tf_data(g_sep[0],B,s1,s2).round(), masks[0])
            pred_masks[1] = torch.mul(tf_data(g_sep[1],B,s1,s2).round(), masks[1])
            pred_masks[2] = torch.mul(tf_data(g_sep[2],B,s1,s2).round(), masks[2])
            pred_masks[3] = torch.mul(tf_data(g_sep[3],B,s1,s2).round(), masks[3])
        else:
...
def calc_metrics(batch_data, outputs, args):
    ...
    for j in range(B):
        ...
        for n in range(N):
            ...
            preds_wav[n] = istft_reconstruction(pred_mag, phase_mix[j, 0], hop_length=args.stft_hop)+ 1e-6
