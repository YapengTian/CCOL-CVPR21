...
from .audio_net import Unet, VGGish, ANet
from .vision_net import ResnetFC, ResnetDilated, Resnet
from .grounding_net import Grounding
...
class ModelBuilder():
    ...
    def build_sound(self, arch='unet6', fc_dim=64, weights=''):
        # 2D models
        if arch == 'unet5':
    ...
    def build_grounding(self, arch, weights=''):
        if arch == 'base':
            net = Grounding()
        else:
            raise Exception('Architecture undefined!')
